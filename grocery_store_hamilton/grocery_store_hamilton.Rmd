...
hamilton_basemap <- read_osm(grocery_DA, type = "esri-shaded")
```
...
The hurdle model has a McFadden pseudo $R^2$ of `r round(pR2(m2)[[4]], digits = 2)`, while the zero-inflated negative binomial regression model has a McFadden pseudo $R^2$ of `r round(pR2(m1)[[4]], digits = 2)`. Table \ref{tab:regression_results} below presents the regression results.
...
p1 <- plot_predictions(m1, condition = c("Freq_lag", "hsr_level")) +
  ...
  labs(x = "Spatial Lag of Number of Grocery Stores",
       y = "Number of Grocery Stores") +
  theme_bw()

p2 <- plot_predictions(m1, condition = c("log_area", "hsr_level")) +
  scale_color_manual(name = "Number of\nHSR Bus Stops",
                     values = c(
                       "low" = "#a6bddb",
                       "mid" = "#3690c0",
  ...
  labs(x = "Natural Log of Area Size in Square Kilometres",
       y = "Number of Grocery Stores") +
  theme_bw()

p3 <- p1 + p2 + plot_layout(guides = "collect")

ggsave("./images/margin.png", p3, width = 8, height = 3, dpi = 300)
```

```{r, fig.cap="\\label{fig:margin}Conditional Prediction Plot for the Zero-inflated Negative Binomial Regression Model", out.height="2.4in", out.width="6.4in", fig.align='center'}
